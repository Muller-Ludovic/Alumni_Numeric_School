<?php include 'header_p.php';?>

<main>
  <hr class="hr">
  <div class="container-fluid actu">
  <div class="divh2"><h2><strong>Offres d'emploi</strong></h2></div>
  <hr>
  <div class="row justify-content-center">
    <div class="card text-center col-sm-3 mx-4">
      <img src="../images/board.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Card title</h5>
        <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
        <a href="detail_offre.php" class="btn col-10 btn-primary">Go somewhere</a>
      </div>
    </div>
    <div class="card text-center col-sm-3 mx-4">
      <img src="../images/board.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Card title</h5>
        <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
        <a href="#" class="btn col-10 btn-primary">Go somewhere</a>
      </div>
    </div>
      <div class="card text-center col-sm-3 mx-4">
      <img src="../images/board.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Card title</h5>
        <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
        <a href="#" class="btn col-10 btn-primary">Go somewhere</a>
      </div>
    </div>
  </div>
  <div class="row justify-content-center">
    <div class="card text-center col-sm-3 mx-4">
      <img src="../images/board.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Card title</h5>
        <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
        <a href="#" class="btn col-10 btn-primary">Go somewhere</a>
      </div>
    </div>
    <div class="card text-center col-sm-3 mx-4">
      <img src="../images/board.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Card title</h5>
        <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
        <a href="#" class="btn col-10 btn-primary">Go somewhere</a>
      </div>
    </div>
      <div class="card text-center col-sm-3 mx-4">
      <img src="../images/board.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Card title</h5>
        <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
        <a href="#" class="btn col-10 btn-primary">Go somewhere</a>
      </div>
    </div>
  </div>
  <div class="row justify-content-center">
    <div class="card text-center col-sm-3 mx-4">
      <img src="../images/board.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Card title</h5>
        <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
        <a href="#" class="btn col-10 btn-primary">Go somewhere</a>
      </div>
    </div>
    <div class="card text-center col-sm-3 mx-4">
      <img src="../images/board.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Card title</h5>
        <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
        <a href="#" class="btn col-10 btn-primary">Go somewhere</a>
      </div>
    </div>
      <div class="card text-center col-sm-3 mx-4">
      <img src="../images/board.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Card title</h5>
        <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
        <a href="#" class="btn col-10 btn-primary">Go somewhere</a>
      </div>
    </div>
  </div>
  <div class="row justify-content-center">
    <div class="card text-center col-sm-3 mx-4">
      <img src="../images/board.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Card title</h5>
        <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
        <a href="#" class="btn col-10 btn-primary">Go somewhere</a>
      </div>
    </div>
    <div class="card text-center col-sm-3 mx-4">
      <img src="../images/board.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Card title</h5>
        <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
        <a href="#" class="btn col-10 btn-primary">Go somewhere</a>
      </div>
    </div>
      <div class="card text-center col-sm-3 mx-4">
      <img src="../images/board.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Card title</h5>
        <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
        <a href="#" class="btn col-10 btn-primary">Go somewhere</a>
      </div>
    </div>
  </div><br><br><br>
</div>
      <hr class="hr"><br><br><br>
</main>

<?php include 'footer_p.php';?>